...
- DATA CARDS (the strongest scenes — reach for them whenever the content qualifies, respecting each card's per-video cap):
  ...
  · "photo_stack" for a run of 2-4 related photos flipped through like prints, ONE at a time (slot_photo_1..slot_photo_4 image requests; 3 and 4 are optional).
  · "custom_card" — THE LAST RESORT, at most 2 per video. ONLY when the beat needs a specific THING drawn that no other card can express: a boarding pass, a chat exchange, a scoreboard, a nutrition label, a keyboard layout, a parking ticket. slot_custom {content_type: "custom_html", html: a self-contained fragment (inline <svg> allowed; no <script>, no <img>, no links, nothing external), css?: a few class-selector rules (no @media/@keyframes/@import), heading?, caption?}. If ANY other card fits, use that card — it is typeset, animated and tested, and this is not; never use it for a chart, a comparison, a list, a process, an equation or a photo. Colour with var(--accent)/var(--text)/var(--muted)/var(--panel)/var(--line) and set type with var(--font-display)/var(--font-body)/var(--font-mono); flat only. Author against a 1000px-wide canvas, and make it TALL rather than wide in 9:16. NEVER write css animations — put data-at="0.35" (a 0..1 point in the scene) or data-word="gate" (land on that narration word) on elements that should arrive one at a time, optionally with data-anim="fade|rise|pop|slide|grow".
  · "cinematic_card" for the video's KEY EXPLAINING beats — the moment the viewer must understand how the mechanism works, why the result happens, or how 3-5 pieces relate. slot_cinematic {content_type: "cinematic", brief: ONE sentence naming what must become clear and the pieces involved in the order the narration introduces them ("how a vaccine trains the body: the virus, a harmless copy of its spike, the antibodies, the memory cells"), heading?: <=60}. A later pass stages it in depth (each piece arrives out of focus as the narrator names it, the camera pushes in, the rest blurs, then a wide sharp view), so write that beat's narration to NAME each piece in turn over 9-14 seconds. Several per video are fine, but together at most 30% of the runtime and NEVER two in a row; never for a hook, a transition, a list of tips or a lone number.
  · "image_grid" when a beat needs 3-6 pictures on screen AT ONCE so the viewer compares them — four examples of the same thing, the six variants, the three species (slot_image_1..slot_image_6 image requests, 4-6 optional; each cell also takes label: a 1-3 word caption, and slot_image_1 may carry heading?). They land one at a time as the narration names them and all stay visible. Every cell must be a DIFFERENT subject — a repeated one is dropped. One picture is single_focus, two is split_side_by_side, a sequence shown one at a time is photo_stack. At most 1 per video.
...
- THE MEDIA BRIEF: every image/video asset_request also carries "search_query", "media_kind" and "guidance", because the storyboard shows the user free stock results for the slot and they must be able to judge them. "search_query" = 2-4 PLAIN words that would find this on a stock site ("busy trading floor", "solar panels desert") — nouns, never prompt language. "media_kind" = "image" when a still says it, "video" when the beat is ABOUT movement (a machine running, a crowd, liquid pouring), "either" when both work; keep "video" to at most a third of the media slots. "guidance" = ONE sentence written TO THE USER saying what shot works here and what to avoid ("A wide daylight shot of a real newsroom with people at desks — not a posed stock handshake, nothing with readable text on screens.").
  ...
  "scenes": [
    {
      ...
      "slots": {
        "slot_left": {
          ...
          "asset_request": { "description": "GTA V world map screenshot", "search_query": "video game map", "media_kind": "image", "guidance": "A clean top-down screenshot of the in-game map with the legend visible — not concept art." }
        },
        "slot_right": {
