...
class AnalyzeExplainerScriptJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            try {
                \Modules\Project\Support\ExplainerAssetRehome::apply($this->project, $assetPlan, $oldAssetRows);
            ...
            // A full re-analysis rebuilds the board from scratch, so the last
            // AI-revision's per-scene "changed / added" markers point at
            // scenes that no longer exist. The LOG of what the user asked for
            // survives — that is history, not state.
            if (is_array($settings['revision'] ?? null)) {
                $settings['revision']['state'] = 'idle';
